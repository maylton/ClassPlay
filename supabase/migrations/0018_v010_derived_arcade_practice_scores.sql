-- ClassPlay v0.10 — derived Arcade practice scores
--
-- Centralizes the runtime-derived leaderboard policy for Boss Battle, Bubble
-- Burst, Grammar Runner and Phrase Forge. Derived modes remain absent from
-- activity_games; score submission is allowed only when the source activity is
-- unlisted and contains enough source content to unlock that runtime mode.
...
create policy "unlisted practice scores link submit"
  ...
  with check (
    ...
    and (
      ...
      or (
        practice_scores.game_type in ('boss-battle', 'bubble-burst', 'grammar-runner')
        and (
          (
            3 <= (
              select count(*)
