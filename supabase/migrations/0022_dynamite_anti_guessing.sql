...
declare
  ...
  v_penalty_seconds constant integer := 3;
  v_penalized_started_at timestamptz;
  v_force_explosion boolean := false;
  v_normalized_answer text;
  v_attempted_answers jsonb := '[]'::jsonb;
begin
  select *
  into v_player
  from public.players
  ...
  v_normalized_answer := public.classplay_normalize(p_answer_text);
  v_correct := v_normalized_answer
  ...
  if v_correct then
    ...
    v_attempted_answers := coalesce(v_session.current_question->'dynamiteAttemptedAnswers', '[]'::jsonb);

    -- Repeated clicks on the same wrong option must not consume another attempt.
    if exists (
      select 1
      from jsonb_array_elements_text(v_attempted_answers) as attempted(value)
      where attempted.value = v_normalized_answer
    ) then
      return jsonb_build_object(
        'correct', false,
        'passed', false,
    ...
    v_wrong_attempts := coalesce((v_session.current_question->>'dynamiteWrongAttempts')::integer, 0) + 1;
    v_attempted_answers := v_attempted_answers || jsonb_build_array(v_normalized_answer);
    v_penalized_started_at := v_session.round_started_at - make_interval(secs => v_penalty_seconds);
    v_force_explosion := v_wrong_attempts >= 3
      or now() >= v_penalized_started_at + make_interval(secs => v_timer_seconds);

    -- On the third wrong answer (or if the penalty consumes the fuse), make the
    -- authoritative timer expired so the existing host elimination flow fires.
    if v_force_explosion then
      v_penalized_started_at := now() - make_interval(secs => v_timer_seconds + 1);
    end if;

    update public.game_sessions
    set round_started_at = v_penalized_started_at,
        current_question = current_question || jsonb_build_object(
          'startedAt', v_penalized_started_at,
          'dynamiteWrongAttempts', v_wrong_attempts,
          'dynamiteAttemptedAnswers', v_attempted_answers
        ),
        updated_at = now()
    where id = v_session.id;
  ...
  return jsonb_build_object(
    'correct', v_correct,
    'passed', v_correct,
    'timeUp', v_force_explosion,
    'points', v_points,
    'score', v_player.score,
    'alreadyPassed', false,
    'penaltySeconds', case when v_correct then 0 else v_penalty_seconds end,
    'wrongAttempts', case when v_correct then 0 else v_wrong_attempts end
  );
end;
$$;
