...
assert.match(liveHost, /dynamiteState\.order/, "The host must render the persisted Dynamite turn order.");
...
assert.match(liveStudent, /dynamite-shake/, "Wrong Dynamite attempts must trigger the requested shake feedback.");
assert.match(liveStudent, /StudentDynamiteQueue/, "Students must be able to see the turn order and prepare for the next turn.");
