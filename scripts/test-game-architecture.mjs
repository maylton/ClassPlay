...
const hunt = await readFile(new URL("../src/lib/word-hunt-engine.ts", import.meta.url), "utf8");
const typing = await readFile(new URL("../src/lib/typing-rush-engine.ts", import.meta.url), "utf8");
...
assert.doesNotMatch(hub, /import \{ (?:BossBattleGame|BubbleBurstGame|FlashcardsGame|GrammarRunnerGame|PhraseForgeGame|TowerStackGame|WordHuntGame|TypingRushGame)/, "GameHub must not rebuild the renderer registry with per-game imports");
assert.doesNotMatch(hub, /mode === "(?:flashcards|memory|matching|sentence-builder|gap-fill|quiz|space-blaster|word-maze|boss-battle|bubble-burst|grammar-runner|phrase-forge|tower-stack|word-hunt|typing-rush)"/, "GameHub must not rebuild the renderer registry with mode branches");
assert.doesNotMatch(hub, /const ARCADE_MODES/, "Arcade classification should live in game-catalog");
assert.doesNotMatch(hub, /bossReady|bubbleReady|runnerReady|forgeReady|towerReady|huntReady|typingReady/, "derived Arcade readiness should be generic");

for (const game of ["flashcards", "memory", "matching", "sentence-builder", "gap-fill", "quiz", "space-blaster", "word-maze", "boss-battle", "bubble-burst", "grammar-runner", "phrase-forge", "tower-stack", "word-hunt", "typing-rush"]) {
...
for (const derived of ["boss-battle", "bubble-burst", "grammar-runner", "phrase-forge", "tower-stack", "word-hunt", "typing-rush"]) {
  assert.doesNotMatch(authorable, new RegExp(derived), `${derived} must stay runtime-derived`);
}

for (const [name, source] of [["Boss Battle", boss], ["Bubble Burst", bubble], ["Grammar Runner", runner], ["Tower Stack", tower], ["Word Hunt", hunt], ["Typing Rush", typing]]) {
