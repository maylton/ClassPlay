...
assert.doesNotMatch(hub, /import \{ (?:BossBattleGame|BubbleBurstGame|FlashcardsGame|GrammarRunnerGame|PhraseForgeGame)/, "GameHub must not rebuild the renderer registry with per-game imports");
assert.doesNotMatch(hub, /mode === "(?:flashcards|memory|matching|sentence-builder|gap-fill|quiz|space-blaster|word-maze|boss-battle|bubble-burst|grammar-runner|phrase-forge)"/, "GameHub must not rebuild the renderer registry with mode branches");
assert.doesNotMatch(hub, /const ARCADE_MODES/, "Arcade classification should live in game-catalog");
assert.doesNotMatch(hub, /bossReady|bubbleReady|runnerReady|forgeReady/, "derived Arcade readiness should be generic");

for (const game of ["flashcards", "memory", "matching", "sentence-builder", "gap-fill", "quiz", "space-blaster", "word-maze", "boss-battle", "bubble-burst", "grammar-runner", "phrase-forge"]) {
...
for (const derived of ["boss-battle", "bubble-burst", "grammar-runner", "phrase-forge"]) {
  assert.doesNotMatch(authorable, new RegExp(derived), `${derived} must stay runtime-derived`);
}

for (const [name, source] of [["Boss Battle", boss], ["Bubble Burst", bubble], ["Grammar Runner", runner]]) {
