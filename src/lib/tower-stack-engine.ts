...
export type TowerPlacementResult = TowerBlockGeometry & {
  landed: boolean;
  perfect: boolean;
  recovered: boolean;
  points: number;
...
export function chooseTowerStackSource(
  kind: ActivityKind,
  quizCount: number,
  gapCount: number,
...
export function buildTowerStackRounds(
...
export function resolveTowerPlacement(
  ...
  if (overlap < TOWER_STACK_MIN_BLOCK_WIDTH) {
  ...
  return {
    ...
    perfect: false,
    recovered: false,
    points: 35 + Math.round(width * 1.6),
  };
}

export function towerHeightMeters(floors: number) {
  return Math.round(Math.max(0, floors) * 4.2);
}
