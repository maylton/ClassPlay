...
export function AuthClient({ nextPath = "/dashboard" }: { nextPath?: string }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (mode === "signup") {
        const { error } = await supabase.auth.signUp({ email, password, options: { data: { display_name: name.trim() || "Teacher" }, emailRedirectTo: `${window.location.origin}/auth/callback?next=${encodeURIComponent(nextPath)}` } });
  ...
  return (
    <main className="auth-screen">
      <section className="auth-card">
        ...
        <p>Your activities sync to the cloud while students still join live games without accounts.</p>
        <form onSubmit={submit} className="auth-form">
          {mode === "signup" && <label className="field"><span>Your name</span><input value={name} onChange={(event) => setName(event.target.value)} placeholder="Teacher Maylton" /></label>}
        ...
        <button className="auth-switch" onClick={() => { setMode((current) => current === "signin" ? "signup" : "signin"); setMessage(""); }}>{mode === "signin" ? "New to ClassPlay? Create an account" : "Already have an account? Sign in"}</button>
        <a href="/dashboard" className="auth-local-link">Continue with local mode when cloud is not configured</a>
      </section>
    </main>
  );
}
