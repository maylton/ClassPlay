...
export function LiveSessionSetup({ activityId }: { activityId: string }) {
    ...
    return (
      <main className="cloud-setup-screen">
        <section className="cloud-setup-card">
          <span className="cloud-setup-icon">☁</span>
          ...
          <Link href={`/play/${activityId}`} className="button button-primary">← Back to local games</Link>
        </section>
      </main>
    );
  }
  ...
  if (error || !activity) return <main className="not-found"><span>⚠</span><h1>Could not prepare room</h1><p>{error}</p><Link href="/dashboard" className="button button-primary">Back to library</Link></main>;
      ...
      <section className="live-setup-grid">
        ...
        <article className="live-options-card">
          ...
          <div className="mode-segmented">
            <button className={mode === "individual" ? "active" : ""} onClick={() => setMode("individual")}><b>👤 Individual</b><small>Each student earns their own score</small></button>
            <button className={mode === "team" ? "active" : ""} onClick={() => setMode("team")}><b>👥 Teams</b><small>Students are balanced automatically</small></button>
          </div>
          ...
          <div className="live-setting-summary"><span>{settings.timerEnabled ? `⏱ ${settings.timerSeconds}s timer` : "∞ No timer"}</span><span>{settings.leaderboardEnabled ? "🏆 Leaderboard on" : "🙈 Leaderboard off"}</span><span>{settings.readAloud ? "🔊 Read aloud" : "🔇 Manual audio"}</span></div>
          <button className="button button-primary button-large start-live-button" disabled={busy} onClick={() => void create()}>{busy ? "Creating room…" : "Create live room →"}</button>
        </article>
      </section>
    </main>
  );
