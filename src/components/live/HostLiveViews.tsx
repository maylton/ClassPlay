    ...
    <main className="host-room lobby-screen">
      ...
      <section className="lobby-layout">
        <div className="join-panel">
          <span className="eyebrow">{isWildcardGrid ? "Phones are optional" : "Students join at"}</span><strong className="join-domain">{joinUrl.replace(/^https?:\/\//, "")}</strong>
          ...
          <p>{isWildcardGrid ? "Play entirely from the projector, or let students scan the QR code for team and score updates on their phones." : "Scan the QR code or enter the six-digit room code. No student account is required."}</p>
        ...
        <div className="lobby-players-panel">
          ...
          <div className="lobby-player-grid">
            ...
            {!players.length && <div className="empty-lobby"><span><AppIcon name={isWildcardGrid ? "display" : "people"} /></span><strong>{isWildcardGrid ? "Projector-only is ready." : "Waiting for students…"}</strong><p>{isWildcardGrid ? "You can start now. Phones are an optional companion for this game." : "Names will appear here as they join."}</p></div>}
          </div>
          ...
          {isWildcardGrid && <div className="dynamite-lobby-rule wildcard-lobby-rule"><AppIcon name="grid-3x3-gap-fill" /><div><b>{session.settings.wildcardGridSize ?? 12} tiles · {teams.length} teams · {session.settings.wildcardGridIntensity === "chaos" ? "Chaos" : "Balanced"}</b><span>Teams answer out loud. You control the whole game here; connected phones only mirror team status.</span></div></div>}
          ...
          {isWildcardGrid && <small className="dynamite-minimum">No phones required — the physical classroom teams can play entirely from the projector.</small>}
        </div>
      </section>
    </main>
  );
